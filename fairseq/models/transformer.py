...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        #args for perturbations
        parser.add_argument('--sampling-method', default='none', choices=['none', 'worddrop', 'similarity', 'uniform', 'condprob', 'worddrop_with_sim', 'worddrop_with_sim_enc_drop', 'worddrop_with_uni', 'worddrop_with_uni_enc_drop'],
                            help='sampling method for perturbations. condprob means sampling as parallel scheduled sampling with step 1')
        parser.add_argument('--enc-replace-rate', default=0., type=float,
                            help='replacement rate for encoder-side tokens')
        parser.add_argument('--dec-replace-rate', default=0., type=float,
                            help='replacement rate for decoder-side tokens')
        parser.add_argument('--decay-val', default=1000, type=int,
                            help='hyper parameter for inverse sigmoid decay in token replacement')
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        if args.share_all_embeddings:
            ...
            encoder_embed_tokens = cls.build_embedding(
                args, src_dict, args.encoder_embed_dim, mask_file_prefix + '_encoder.pt', args.encoder_embed_path
            )
            ...
            decoder_embed_tokens = cls.build_embedding(
                args, tgt_dict, args.decoder_embed_dim, mask_file_prefix + '_decoder.pt', args.decoder_embed_path
            )
    ...
    @classmethod
    def build_embedding(cls, args, dictionary, embed_dim, mask_file=None, path=None):
        num_embeddings = len(dictionary)
        padding_idx = dictionary.pad()
        if args.one_emb is None:
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.enc_replace_rate = args.enc_replace_rate
        self.decay_val = args.decay_val
    ...
    def forward_embedding(self, src_tokens):
        ...
        x = replace_input_tokens(x, self.embed_tokens, self.sampling_method, self.enc_replace_rate, 
                                self._num_updates, self.decay_val, self.training)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self._num_updates = 0
        self.sampling_method = args.sampling_method
        self.dec_replace_rate = args.dec_replace_rate
        self.decay_val = args.decay_val
    ...
    def extract_features(
        ...
        #replace embeddings (or word dropout)
        if not self.training or self.sampling_method != 'condprob':
            x = replace_input_tokens(x, self.embed_tokens, self.sampling_method, self.dec_replace_rate, 
                                    self._num_updates, self.decay_val, self.training, decoder=True, decoder_out=None)
        else:
            self.eval()
            decout, extra = self.extract_features(prev_output_tokens, encoder_out, tgt_lengths,
                                                incremental_state, alignment_layer, alignment_heads)
            decout = self.output_layer(decout)
            self.train()
            x = replace_input_tokens(x, self.embed_tokens, self.sampling_method, self.dec_replace_rate, 
                                    self._num_updates, self.decay_val, self.training, decoder=True, decoder_out=decout)
        ...
        if self.adaptive_softmax is None:
            ...
            if self.share_input_output_embed:
                if self.embed_tokens.weight.size(0) == 1:
                    ...
                    return F.linear(features, output_matrix)
                else:
...
def sample_tokens(base_emb, embed_module):
    ...
    logit = F.log_softmax(F.linear(base_emb, embed_module.weight), dim=-1)
    noise = logit.clone()
    maxind = (logit - (-noise.uniform_().log()).log()).argmax(dim=-1)
    return maxind.detach()

def replace_input_tokens(base_emb, embed_module, sampling_method, replace_rate, updates, inverse_val, training=False, decoder=False, decoder_out=None):
    #shape of base emb: B x T x C
    if not training or not replace_rate > 0 or (sampling_method == 'condprob' and not decoder):
        return base_emb
    if sampling_method == 'worddrop':
        ...
        #compute replace rate and flag keeping base embeddings
        decay = inverse_val / (inverse_val + np.exp(updates / inverse_val))
        keep_flag = base_emb.new_full((base_emb.size(0), base_emb.size(1)), max(decay, 1 - replace_rate))
        keep_flag = keep_flag.bernoulli()
        if decoder:
            #keep bos tag in decoder side
            keep_flag[:, 0] = 1
        ...
        return base_emb * keep_flag + sampled_embeds * (1 - keep_flag)
    elif sampling_method == 'condprob':
        #replace embeddings based on decoder output as scheduled sampling (shape: B x T x C)
        #compute replace rate and flag keeping base embeddings
        decay = inverse_val / (inverse_val + np.exp(updates / inverse_val))
        keep_flag = base_emb.new_full((base_emb.size(0), base_emb.size(1)), max(decay, 1 - replace_rate))
        keep_flag = keep_flag.bernoulli()
        if decoder:
            ...
            keep_flag[:, 0] = 1
        keep_flag = keep_flag.unsqueeze(2)
        #sampling for word index
        logit = F.log_softmax(decoder_out, dim=-1)
        noise = logit.clone()
        maxind = ((logit - (-noise.uniform_().log()).log()).argmax(dim=-1)).detach() #B x T
        maxind[:, 1:] = maxind[:, :-1] #shift index because maxind based on decoder output
        sampled_embeds = embed_module(maxind)
        return base_emb * keep_flag + sampled_embeds * (1 - keep_flag)
    elif sampling_method == 'worddrop_with_sim':
        #compute replace rate and flag keeping base embeddings
        decay = inverse_val / (inverse_val + np.exp(updates / inverse_val))
        keep_flag = base_emb.new_full((base_emb.size(0), base_emb.size(1)), max(decay, 1 - replace_rate))
        keep_flag = keep_flag.bernoulli()
        #word dropout
        keep_flag_worddrop = base_emb.new_full((base_emb.size(0), base_emb.size(1)), 1 - replace_rate)
        keep_flag_worddrop = keep_flag_worddrop.bernoulli()
        if decoder:
            #keep bos tag in decoder side
            keep_flag[:, 0] = 1
            keep_flag_worddrop[:, 0] = 1
        keep_flag = keep_flag.unsqueeze(2)
        #sampling embeddings for replacement
        sampled_tokens = sample_tokens(base_emb, embed_module)
        sampled_embeds = embed_module(sampled_tokens)
        return keep_flag_worddrop.unsqueeze(2) * (base_emb * keep_flag + sampled_embeds * (1 - keep_flag))
    elif sampling_method == 'worddrop_with_sim_enc_drop':
        #word dropout
        keep_flag_worddrop = base_emb.new_full((base_emb.size(0), base_emb.size(1)), 1 - replace_rate)
        keep_flag_worddrop = keep_flag_worddrop.bernoulli()
        if decoder:
            ...
            return base_emb * keep_flag_worddrop.unsqueeze(2)
    elif sampling_method == 'worddrop_with_uni':
        #compute replace rate and flag keeping base embeddings
        decay = inverse_val / (inverse_val + np.exp(updates / inverse_val))
        keep_flag = base_emb.new_full((base_emb.size(0), base_emb.size(1)), max(decay, 1 - replace_rate))
        keep_flag = keep_flag.bernoulli()
        ...
        sampled_embeds = embed_module(maxind)
        return keep_flag_worddrop.unsqueeze(2) * (base_emb * keep_flag + sampled_embeds * (1 - keep_flag))
    elif sampling_method == 'worddrop_with_uni_enc_drop':
        #word dropout
        keep_flag_worddrop = base_emb.new_full((base_emb.size(0), base_emb.size(1)), 1 - replace_rate)
        keep_flag_worddrop = keep_flag_worddrop.bernoulli()
        if decoder:
